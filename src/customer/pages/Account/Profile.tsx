            ...
            <div className='grid grid-cols-1 lg:grid-cols-3 lg:min-h-[78vh]'>
                <div className="col-span-1 lg:border-r lg:pr-5 py-5 h-full flex flex-row flex-wrap lg:flex-col gap-3">
                    {user?.user?.role === 'ROLE_ADMIN' && (
                        <div
                            onClick={() => handleClick({ name: "admin", path: "/admin" })}
                            className={`${location.pathname === "/admin" ? "bg-primary-color text-white" : ""} px-5 py-3 rounded-md hover:bg-teal-500 hover:text-white cursor-pointer`}
                        >
                            <p>admin</p>
                        </div>
                    )}
